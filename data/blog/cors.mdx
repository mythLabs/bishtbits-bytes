...
> On clicking the "Same-Domain Request" button, we can see that the /simple endpoint is called within the same domain, so no CORS headers are required by the browser.

## Cross Domain Simple Request 
![simple CORS request](/static/images/blog/cors/p4.png)
> On clicking "Cross-Domain Simple", we can see that the /simple endpoint is called, but the domain is https://localhost:4000. Therefore, CORS headers are required by the browser.

## Cross Domain With Preflight
![simple CORS request](/static/images/blog/cors/p5.png)
> On clicking "Cross-Domain Preflight", we can see that a preflight request is made, and if it is successful, the actual API call is performed.
